...
var setRuntimeBindings = (bindings) => {
...
var getBindings = () => runtimeBindings ?? {};

// src/utils/api.ts
async function createStreamReq(content2) {
  ...
  const response = await fetch(`${API_CF_ENDPOINT}/streaming`, {
    ...
    body: JSON.stringify({ content: content2, type: "note", author: "bbki.ng" })
  });
  if (!response.ok) {
...
async function createPost(title, content2) {
  const API_CF_ENDPOINT = requireEnv("API_CF_ENDPOINT");
  const STREAM_API_KEY = requireEnv("STREAM_API_KEY");
...
var package_default = {
  name: "bbbot",
  version: "1.2.0",
  description: "Campfire bot with COC Master AI integration",
  main: "src/worker.ts",
  scripts: {
    dev: "wrangler dev src/worker.ts",
    deploy: "wrangler deploy",
    release: "node build/build.mjs"
  },
  keywords: [],
  author: "",
  license: "ISC",
...
var version = async () => {
  return `bbbot v${package_default.version} running on Campfire.`;
};
var content = async (payload) => {
  const text2 = payload.message.body.plain;
...
async function handleStream(content2) {
  if (!content2) return "Cannot create empty stream.";
  const result = await createStreamReq(content2);
  return `Stream created: <strong>${escapeHtml(result.data.id)}</strong>`;
}
async function handlePost(title, body) {
...
var InvalidDataContentError = class extends AISDKError {
  constructor({
    content: content2,
    cause,
    message = `Invalid data content. Expected a base64 string, Uint8Array, ArrayBuffer, or Buffer, but got ${typeof content2}.`
  }) {
...
function convertDataContentToUint8Array(content2) {
  if (content2 instanceof Uint8Array) {
    return content2;
  }
  if (typeof content2 === "string") {
    try {
      return convertBase64ToUint8Array(content2);
    } catch (error) {
  ...
  if (content2 instanceof ArrayBuffer) {
    return new Uint8Array(content2);
  }
  throw new InvalidDataContentError({ content: content2 });
}
...
function convertPartToLanguageModelPart(part, downloadedAssets) {
  ...
  try {
    content2 = typeof data === "string" ? new URL(data) : data;
  } catch (error) {
    content2 = data;
  }
  if (content2 instanceof URL) {
    if (content2.protocol === "data:") {
      ...
      const downloadedFile = downloadedAssets[content2.toString()];
      if (downloadedFile) {
        normalizedData = downloadedFile.data;
        mimeType != null ? mimeType : mimeType = downloadedFile.mimeType;
      } else {
        normalizedData = content2;
      }
    }
  } else {
    normalizedData = convertDataContentToUint8Array(content2);
  }
  switch (type) {
    case "image": {
...
function convertToCoreMessages(messages, options) {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    const { role, content: content2, experimental_attachments } = message;
    switch (role) {
      case "system": {
        coreMessages.push({
          role: "system",
      ...
      case "assistant": {
        ...
        for (let i2 = 0; i2 <= maxStep; i2++) {
          ...
          coreMessages.push({
            role: "assistant",
            content: [
              ...isLastMessage && content2 && i2 === 0 ? [{ type: "text", text: content2 }] : [],
              ...stepInvocations.map(
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    switch (type) {
      case "system": {
        ...
        system = block.messages.map(({ content: content2, providerMetadata }) => ({
          type: "text",
          text: content2,
...
var AnthropicMessagesLanguageModel = class {
  ...
  async doGenerate(options) {
    ...
    if (response.content.some((content2) => content2.type === "tool_use")) {
      toolCalls = [];
      for (const content2 of response.content) {
        if (content2.type === "tool_use") {
          toolCalls.push({
            toolCallType: "function",
            toolCallId: content2.id,
            toolName: content2.name,
            args: JSON.stringify(content2.input)
          });
        }
      }
    }
    const reasoning = response.content.filter(
      (content2) => content2.type === "redacted_thinking" || content2.type === "thinking"
    ).map(
      (content2) => content2.type === "thinking" ? {
        type: "text",
        text: content2.thinking,
        signature: content2.signature
      } : {
        type: "redacted",
        data: content2.data
      }
...
var getClanInfoTool = (token, proxyKey) => tool({
  ...
  execute: async (args) => {
    try {
      ...
      return JSON.stringify({
        error: error.message || "Failed to fetch clan info"
      });
    }
  }
});
...
var getCurrentWarTool = (token, proxyKey) => tool({
  ...
  execute: async (args) => {
    try {
      ...
      return JSON.stringify({
        error: error.message || "Failed to fetch war info"
      });
    }
  }
});

// src/coc-master/coc-wiki-importer.ts
var COC_PY_STATIC_DATA_URL = "https://raw.githubusercontent.com/mathsman5133/coc.py/master/coc/static/static_data.json";
var DEFAULT_GUIDE_URL = "https://raw.githubusercontent.com/bbbottle/bottle/main/apps/backend/data/coc-wiki-guide.json";
var KV_KEY_GUIDE = "coc-wiki:guide:v1";
var KV_KEY_GAME_DATA = "coc-wiki:game-data:v1";
var DEFAULT_CACHE_TTL_SECONDS = 86400 * 7;
var MemoryKV = class {
  ...
  async put(key, value, options) {
    const expiresAt = options?.expirationTtl ? Date.now() + options.expirationTtl * 1e3 : null;
    this.store.set(key, { value, expiresAt });
  }
};
var CocWikiDataImporter = class {
  guideUrl;
  constructor(guideUrl) {
  ...
  async sync(kv) {
    ...
    try {
      ...
      gameDataUnitsCount = gameData.buildings.length + gameData.troops.length + gameData.heroes.length + gameData.spells.length + gameData.traps.length;
  ...
  async fetchAndRefineGameData() {
    ...
    const pickFields = (arr) => (arr || []).filter((item) => item && typeof item === "object").map((item) => ({
      name: String(item.name || ""),
      type: String(item.type || ""),
      village: String(item.village || "home"),
      levels: (item.levels || []).map((lv) => ({
        level: Number(lv.level || 0),
        build_cost: lv.build_cost != null ? Number(lv.build_cost) : void 0,
        build_time: lv.build_time != null ? Number(lv.build_time) : void 0,
        required_townhall: lv.required_townhall != null ? Number(lv.required_townhall) : void 0,
        hitpoints: lv.hitpoints != null ? Number(lv.hitpoints) : void 0,
        dps: lv.dps != null ? Number(lv.dps) : void 0,
        housing_space: lv.housing_space != null ? Number(lv.housing_space) : void 0,
        training_time: lv.training_time != null ? Number(lv.training_time) : void 0
      }))
    })).filter((u) => u.name);
    return {
...
var cocWikiDataImporter = new CocWikiDataImporter();

// src/coc-master/coc-wiki-service.ts
var GENERIC_ADVICE = "\u8FD9\u662F\u4E00\u4E2A\u5F88\u597D\u7684\u95EE\u9898\u3002\u4F5C\u4E3A COC \u987E\u95EE\uFF0C\u6211\u5EFA\u8BAE\u4F60\uFF1A\n1. \u660E\u786E\u5F53\u524D\u5927\u672C\u7B49\u7EA7\u548C\u76EE\u6807\n2. \u4F18\u5148\u5347\u7EA7\u8FDB\u653B\u7C7B\u5EFA\u7B51\uFF08\u5B9E\u9A8C\u5BA4\u3001\u5175\u8425\u3001\u8BAD\u7EC3\u8425\uFF09\n3. \u4FDD\u6301\u82F1\u96C4\u6301\u7EED\u5347\u7EA7\n4. \u591A\u89C2\u5BDF\u9AD8\u672C\u73A9\u5BB6\u7684\u9635\u578B\u548C\u6253\u6CD5\n5. \u79EF\u6781\u53C2\u4E0E\u90E8\u843D\u6218\u548C\u90E8\u843D\u7ADE\u8D5B\u83B7\u53D6\u5956\u52B1\n\n\u4F60\u53EF\u4EE5\u544A\u8BC9\u6211\u66F4\u5177\u4F53\u7684\u9700\u6C42\uFF0C\u6BD4\u5982\u5175\u79CD\u642D\u914D\u3001\u9632\u5FA1\u5E03\u5C40\u3001\u8D44\u6E90\u7BA1\u7406\u7B49\u3002";
var DATA_UNAVAILABLE_ADVICE = "\u6570\u636E\u6682\u65F6\u4E0D\u53EF\u7528\uFF0C\u6B63\u5728\u4ECE\u5916\u90E8\u6570\u636E\u6E90\u540C\u6B65\u6700\u65B0\u4FE1\u606F\u3002\u8BF7\u7A0D\u540E\u518D\u8BD5\uFF0C\u6216\u8BE2\u95EE\u5176\u4ED6\u95EE\u9898\u3002";
var CocWikiService = class {
  async query(kv, topic, townHallLevel) {
  ...
  findMatchingTopic(guide, lowerTopic) {
  ...
  enrichAdvice(template, thLevel, gameData) {
...
var cocWikiService = new CocWikiService();

// src/coc-master/tools/get-development-guide.tool.ts
var getDevelopmentGuideTool = (kv) => tool({
  description: "\u67E5\u8BE2\u6743\u5A01\u53D1\u5C55\u5EFA\u8BAE\uFF0C\u5305\u62EC\u5347\u7EA7\u4F18\u5148\u7EA7\u3001\u5175\u79CD\u642D\u914D\u3001\u9632\u5FA1\u7B56\u7565\u3001\u8D44\u6E90\u7BA1\u7406\u3001\u90E8\u843D\u6218\u6280\u5DE7\u3001\u82F1\u96C4\u5347\u7EA7\u89C4\u5212\u7B49",
  parameters: external_exports.object({
    topic: external_exports.string().describe(
      '\u67E5\u8BE2\u4E3B\u9898\uFF0C\u5982"\u5347\u7EA7\u4F18\u5148\u7EA7"\u3001"\u5175\u79CD\u642D\u914D"\u3001"\u9632\u5FA1\u5E03\u5C40"\u3001"\u8D44\u6E90\u7BA1\u7406"\u3001"\u90E8\u843D\u6218"\u3001"\u82F1\u96C4\u5347\u7EA7"'
    ),
    townHallLevel: external_exports.number().optional().describe("\u5F53\u524D\u5927\u672C\u7B49\u7EA7\uFF0C\u7528\u4E8E\u63D0\u4F9B\u9488\u5BF9\u6027\u5EFA\u8BAE")
  }),
  execute: async (args) => {
...
// src/coc-master/chat.ts
var createKimiClient = (apiKey) => createAnthropic({
  baseURL: "https://api.kimi.com/coding/v1/",
  apiKey
});
var SYSTEM_PROMPT = `\u4F60\u662F\u8D44\u6DF1\u300A\u90E8\u843D\u51B2\u7A81\u300B(Clash of Clans) \u6E38\u620F\u7B56\u7565\u4E13\u5BB6\u3002
...
`;
async function askCocMaster(playerTag, question, config) {
...
var COC_TAG_PATTERN = /^#([A-Z0-9]{6,12})\s*(.*)$/i;
function getKV() {
  const bindings = getBindings();
  if (bindings.SESSION_KV) {
    return bindings.SESSION_KV;
  }
  return new MemoryKV();
}
function isCocQuery(text2) {
  return COC_TAG_PATTERN.test(text2.trim());
}
var cocMaster = async (payload) => {
  const text2 = payload.message.body.plain.trim();
  const match = text2.match(COC_TAG_PATTERN);
  if (!match) {
